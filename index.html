          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-purple-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-800 mt-1">₹{totalTopup}</p>
              </div>
          ...
          {currentPage === 'dashboard' && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">📅 Salary Date</label>
                  <input
                    type="date"
                    value={salaryDate}
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">📆 EMI Due Date</label>
                  <input
                    type="date"
                    value={dueDate}
              ...
              {result && (
                <div className={`mt-8 p-6 rounded-2xl border-2 shadow-lg ${
                  result.risk === 'High' ? 'border-red-300 bg-gradient-to-br from-red-50 to-red-100' :
                  result.risk === 'Medium' ? 'border-yellow-300 bg-gradient-to-br from-yellow-50 to-yellow-100' :
                  ...
                  <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 space-y-2">
                    <p className="text-gray-800 font-medium"><strong>📋 Reminder:</strong> {result.reminder}</p>
                    {result.topup > 0 && (
                      <p className="text-gray-800 font-medium"><strong>💵 Suggested Top-Up:</strong> ₹{result.topup}</p>
                    )}
                  </div>
                </div>
              )}
            ...
            <div>
              ...
              {emis.length === 0 ? (
                ...
                <div className="space-y-4">
                  {emis.map((e, idx) => (
                    <div key={idx} className={`p-6 rounded-2xl border-2 hover:shadow-xl transition-all ${
                      e.risk === 'High' ? 'border-red-200 bg-red-50' :
                      e.risk === 'Medium' ? 'border-yellow-200 bg-yellow-50' :
                      'border-green-200 bg-green-50'
                    }`}>
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg ${
                            e.risk === 'High' ? 'bg-red-200 text-red-700' :
                            e.risk === 'Medium' ? 'bg-yellow-200 text-yellow-700' :
                            'bg-green-200 text-green-700'
                          }`}>
                            <DollarSignIcon />
                          </div>
                          <div>
                            <span className="font-bold text-xl text-gray-800">₹{e.emi}</span>
                            <p className="text-sm text-gray-600">Due: {e.dueDate}</p>
                          </div>
                        </div>
                        <span className={`px-4 py-2 rounded-full text-sm font-bold ${
                          e.risk === 'High' ? 'bg-red-200 text-red-800' :
                          e.risk === 'Medium' ? 'bg-yellow-200 text-yellow-800' :
                    ...
                    <div key={idx} className="p-6 rounded-2xl border-2 border-red-300 bg-gradient-to-br from-red-50 to-orange-50 hover:shadow-xl transition-all">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-bold text-xl text-gray-800">EMI ₹{e.emi}</span>
                        </div>
                      ...
                      <div className="bg-white/60 backdrop-blur-sm rounded-lg p-4">
                        <p className="text-gray-800 font-bold text-lg">
                          💵 Suggested Top-Up: <span className="text-red-700">₹{e.topup}</span>
                        </p>
